...
const CommentsSystem = ({ contentId }: Props) => {
  ...
  return (
    <div className={`mt-8 sm:mt-12 p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border ${isKidsMode ? 'bg-orange-50 border-orange-200' : 'bg-deep-slate-800/60 border-deep-slate-border shadow-2xl backdrop-blur-xl'}`}>
      <div className="flex items-center gap-3 mb-8">
        <FaCommentAlt className={isKidsMode ? 'text-orange-500 text-2xl' : 'text-ice-mint text-xl'} />
        <h3 className={`text-lg sm:text-xl font-black ${isKidsMode ? 'text-orange-900' : 'text-white'}`}>التعليقات</h3>
        <span className={`px-3 py-0.5 rounded-full text-xs font-bold ${isKidsMode ? 'bg-orange-200 text-orange-800' : 'bg-ice-mint/10 text-ice-mint'}`}>
          {comments.length}
        </span>
      </div>
        ...
        <textarea
          ...
          className={`
            ...
            ${isKidsMode 
              ...
              : 'bg-deep-slate-900/40 border border-deep-slate-border focus:border-ice-mint text-white placeholder-text-muted'}
          `}
        />
        <button
          type="submit"
          ...
          className={`
            ...
            ${isKidsMode 
              ...
              : 'bg-ice-mint text-deep-slate-900 hover:bg-ice-mint-hover shadow-[0_0_20px_rgba(127,255,212,0.3)]'}
            ...
            {comments.length > 0 ? (
              comments.map((comment, index) => (
                <motion.div
                  ...
                  className={`flex gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border ${isKidsMode ? 'bg-white border-orange-100 shadow-sm' : 'bg-deep-slate-900/20 border-deep-slate-border'}`}
                >
                  <div className="flex-shrink-0">
                    {comment.is_fan ? (
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center relative ${isKidsMode ? 'bg-yellow-400 text-yellow-900 shadow-lg' : 'bg-rating text-black shadow-[0_0_15px_rgba(250,204,21,0.3)]'}`}>
                        <FaCrown className="text-xl" />
                        <span className="absolute -top-1 -right-1 flex h-3 w-3">
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rating opacity-75"></span>
                          <span className="relative inline-flex rounded-full h-3 w-3 bg-rating"></span>
                        </span>
                      </div>
                    ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {comment.is_fan === 1 && (
                        <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${isKidsMode ? 'bg-yellow-100 text-yellow-700' : 'bg-rating/10 text-rating'}`}>
                          Premium Fan 🏆
                        </span>
                      )}
