...
const AdminDashboard = () => {
  ...
  const [updating, setUpdating] = useState(false); // Kept for future potential use or removed if strictly following lint
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [settingsRes, statsRes] = await Promise.all([
        fetch('http://localhost:8000/admin/scrapers/settings', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://localhost:8000/admin/dashboard/stats/overview', { headers: { 'Authorization': `Bearer ${token}` } })
      ]);
