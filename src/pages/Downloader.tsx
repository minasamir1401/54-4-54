        ...
        <div className="min-h-screen bg-[#05070a] text-white pt-32 pb-32 px-6 md:px-12">
            ...
            <div className="max-w-[1200px] mx-auto">
                ...
                >
                    ...
                    <h1 className="text-3xl md:text-6xl lg:text-8xl font-black heading-premium italic tracking-tighter leading-none mb-6">
                        تحميل <span className="text-white/30">&</span> مشاهدة
                    </h1>
                    <p className="text-lg md:text-xl text-slate-500 font-bold italic">قم بتحميل مقاطع الفيديو من جميع المواقع المفضلة لديك بسهولة.</p>
                </motion.header>

                {/* Input Lab Section */}
                <div className="glass-panel p-6 md:p-12 lg:p-20 rounded-[2rem] md:rounded-[4rem] border-white/5 shadow-2xl relative overflow-hidden group mb-16">
                    ...
                    <form onSubmit={handleExtract} className="relative z-10 flex flex-col items-center">
                        <div className="w-full relative mb-6 md:mb-10 group">
                            <input
                                ...
                                className="w-full bg-white/5 border border-white/10 p-4 md:p-6 lg:p-8 rounded-[1.5rem] md:rounded-[2.5rem] outline-none text-right dir-rtl font-black text-lg md:text-2xl placeholder:opacity-20 focus:border-[#7fffd4]/40 transition-all shadow-inner"
                                required
                            />
                            <div className="absolute left-6 md:left-10 top-1/2 -translate-y-1/2 text-xl md:text-2xl text-[#7fffd4] opacity-40 group-focus-within:opacity-100 transition-opacity">
                                <FaLink />
                            </div>
                        </div>
                        ...
                        <button
                            disabled={loading}
                            className="w-full md:w-auto bg-[#7fffd4] text-[#05070a] px-8 md:px-12 lg:px-16 py-4 md:py-6 rounded-2xl md:rounded-3xl font-black flex items-center justify-center gap-4 shadow-[0_20px_40px_rgba(127,255,212,0.3)] hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                        >
                            {loading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {results.map((link, idx) => (
                                    <motion.a
                                        ...
                                        className="glass-panel p-5 md:p-8 rounded-2xl md:rounded-[2.5rem] border-white/5 flex flex-row-reverse items-center justify-between hover:bg-white/5 hover:border-[#7fffd4]/20 transition-all group gap-4"
                                    >
                                        <div className="text-right flex-1 min-w-0">
                                            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1 md:mb-2 block">رابط بجودة</span>
                                            <div className="text-lg md:text-2xl font-black text-white group-hover:text-[#7fffd4] transition-colors truncate">{link.quality || 'رابط مباشر'}</div>
                                        </div>
                                        <div className="w-12 h-12 md:w-16 md:h-16 shrink-0 bg-white/5 rounded-2xl flex items-center justify-center group-hover:bg-[#7fffd4] group-hover:text-[#05070a] transition-all">
                                            <FaDownload className="text-base md:text-xl" />
                                        </div>
                                    </motion.a>
                                ))}
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-32">
                    {[
                        ...
                        <div key={i} className="glass-panel p-6 md:p-10 rounded-2xl md:rounded-[3rem] border-white/5 text-center">
